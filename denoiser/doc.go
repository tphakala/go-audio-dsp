...
//
// The per-bin gain is driven by an estimate of the noise power. Light, Medium
// and Heavy presets map onto the gain floor and aggressiveness, and Params
// exposes every knob: the estimator, the residual gain floor, decision-directed
// smoothing and optional smoothing across frequency bins.
//
// A Denoiser streams: call Process (or ProcessInto) with consecutive chunks of
// any size and Flush (or FlushInto) at the end. One Denoiser serves one stream
// at a time and is not safe for concurrent use; process stereo as two
// Denoisers, one per channel.
//
