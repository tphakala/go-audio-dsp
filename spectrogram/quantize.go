...
// hi must be > lo and neither bound may be NaN or infinite; otherwise nothing is
// written and it returns 0. A NaN element maps to 0.
//
...
func QuantizeColumn(dst []uint8, col []float32, lo, hi float64) int {
	...
	if n == 0 || math.IsNaN(lo) || math.IsNaN(hi) || math.IsInf(lo, 0) || math.IsInf(hi, 0) || hi <= lo {
		return 0
	}
	scale := 255 / (hi - lo)
	for i := range n {
