...
func BenchmarkLoop(m dsl.Matcher) {
	...
	// Pattern 3: for range b.N (no variable). No Suggest: the body is captured
	// as $*body (a NodeSlice) that go-ruleguard renders via go/printer, which
	// panics on the NodeSlice and crashes gocritic on any matching file.
	m.Match(
		`for range $b.N { $*body }`,
	).
		Where(m["b"].Type.Is("*testing.B")).
